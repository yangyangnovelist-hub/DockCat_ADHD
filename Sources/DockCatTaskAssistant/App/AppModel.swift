...
final class AppModel: ObservableObject {
    ...
    private func reconcileMindMapAndTasksOnRestore() -> Bool {
        ...
        if !visibleTasks.isEmpty,
           Self.isEffectivelyEmptyMindMapData(snapshot.mindMapDocument.dataJSON) {
            return synchronizeMindMapFromTasks(force: true)
        }
    ...
    private func applyMindMapDataChange(_ dataJSON: String) -> Bool {
        guard let syncResult = Self.makeTaskSyncResult(from: dataJSON, existingTasks: snapshot.tasks) else {
    ...
    private func synchronizeMindMapFromTasks(force: Bool = false) -> Bool {
        let nextDataJSON = Self.makeMindMapDataJSON(
            from: snapshot.tasks,
    ...
    private static func makeTaskSyncResult(from dataJSON: String, existingTasks: [Task]) -> (tasks: [Task], normalizedDataJSON: String, newTaskIDs: [UUID])? {
        ...
        return (visibleTasks + newlyArchivedTasks + carriedArchivedTasks, normalizedDataJSON, newTaskIDs)
    }

    private static func makeMindMapDataJSON(from tasks: [Task], existingDataJSON: String?) -> String {
        let visibleTasks = tasks.filter { $0.status != .archived }
        let groupedTasks = Dictionary(grouping: visibleTasks) { $0.parentTaskID }

        var payload = existingDataJSON.flatMap(jsonObject(from:)) ?? jsonObject(from: AppSnapshot.empty.mindMapDocument.dataJSON) ?? [:]
    ...
    private static func isEffectivelyEmptyMindMapData(_ dataJSON: String) -> Bool {
